...
well_known_URLs | `string` or `sequence of URLs` | Well-known URLs at which a resource of this entity type can be found. If the value is a string, it is interpreted as a space-deliminated list of URLs. If the value is an sequence, each item is interpreted as a single URL. URLs must be path-absolute - i.e. they must begin with a single '/'. If the enclosing entity has no query paramters, then a single instance of the entity is found at the well_known_URL.
If the enclosing entity has query parameters, then the well_known_URL actually defines a [URI Template](https://tools.ietf.org/html/rfc6570) for a whole `family` of entities defined by the template.
