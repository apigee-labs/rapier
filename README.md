...
[From a JSON perspective, this pattern looks convoluted to us—perhaps someone can offer a explanation of its merits.]
...
In simple cases, OpenAPI can capture the full meaning, but OpenAPI has trouble with some cases, like heterogeneous relationships. The Rapier OpenAPI genenrator
...
The Rapier genenrator adds a couple of custom properties (e.g. ``x-interface) to these definitions—these can be viewed as comments from an OpenAPI perspective.
...
this way because multiple paths may lead to the same `Path Item Object`, and we don't want to have to repeat the definitions multiple times.
