            ...
            if r.status_code == 201:
                if 'Location' in r.headers:
                    if 'ETag' in r.headers:
                        ...
                        raise Exception('server failed to return ETag on POST. URL: %s entity: %s' % (self.self, entity))
                else:
                    raise Exception('server failed to return Location header on POST. URL: %s entity: %s' % (self.self, entity))
            else:
                raise Exception('Unable to create entity in API. status_code: %s Exception-text: %s entity: %s' % (r.status_code, r.text, entity)) 
        else:
            raise Exception('Collection has no self Content-Location')